...
namespace ValueConverters
{
    ...
    public class LevelToColorConverter : IValueConverter
    {
        ...
        {
            // This is one of many cases where conversion back (from brush to grade level) simply
            // isn't necessary. The grade level can be changed either via the UI or through code
            // and we have to be able to convert from grade level to color for display purposes.
            // There is no scenario, however, where a color can be changed by any other means that
            // would require a conversion back to a grade level. For that reason, we can simply
            // throw an exception here.
            //
            // NOTE: If you really wanted the exercise, you could look at the implementation of
            //       "Convert" and simply reverse everything taking into account that a "Brush"
